---
title: "Google 表單即時回饋"
date: "`r Sys.Date()`"
author: "Yongfu, Liao"
output: 
    html_document:
        css: /Users/user/local_depend/style_sheets/style.css
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
	echo = FALSE,
	message = FALSE,
	warning = FALSE
)
source("C:/Users/user/local_depend/R functions/bootstrap_fc.R")
date <- lubridate::with_tz(Sys.time(), tzone = "Asia/Taipei")
library(htmltools)
```


此為 google 表單分數即時回饋的示範平台，請先點選下方「填寫表單」以填寫問卷，問卷最後會要求你輸入 **Token**, 此 **Token** 是用來查詢問卷分數的，請將其記下。


```{r}
# Button trigger modal
button_bs("填寫表單", data_toggle="modal", data_target="#sheet", class="success pull-left")
```

```{r}
# iframe for Google Form
iframe <- HTML('<iframe src="https://docs.google.com/forms/d/e/1FAIpQLSeE-ke7yhlU3XRDIhw7vRL-mnYLi-jTOYF8Pz4ZLINjV53z2A/viewform?embedded=true" style="position: relative; height: 360px; width: 100%;" frameborder="0" marginheight="0" marginwidth="0">載入中…</iframe>')

modal(id="sheet", modal_header = "Google 表單", modal_body=iframe)
```


<br><br><br>

以下按鈕將開啟 DataCamp Light, 為輔助程式語言教學與學習的工具，可用來執行 R 或 Python。下方的例子執行的是 R 語言，並且已預先讀入資料。您僅需於`score()`中輸入金鑰，例如，若您於問卷中 Token 那題填寫「abcde123」，請於下方輸入`score("abcde123")`。若查無結果，請重新整理頁面後再輸入一次。

```{r}
# Button trigger modal: search
button_bs("查詢結果", data_toggle="modal", data_target="#search", class="primary pull-left")
```

```{r}
# iframe for Search
iframe <- HTML('
<script>
function reload() {
document.getElementById("search_iframe").src += "";
} btn2.onclick = reload;
</script>
<iframe id="search_iframe" src="./search.html" style="position: relative; height: 360px; width: 100%;" frameborder="0" marginheight="0" marginwidth="0">載入中…</iframe>
<input type="button" id="btn2" value="refresh" />')

modal(id="search", modal_header = "查詢結果", modal_body=iframe)
```


