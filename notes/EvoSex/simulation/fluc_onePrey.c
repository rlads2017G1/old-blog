...
double dPr1_dt_g(double time, double vr[]); //good year resource
double dPr1_dt_h(double time, double vr[]); //harsh year resource
...
double b1= 0.2; // birth rate of sex
double b2= 0.4; // birth rate of asex
double d1= 0.0015; // death rate of sex
double d2= 0.0015; // death rate of asex
double c= 0.5; // consumption matrix parameter
//double c12= 0.05; // consumption matrix parameter
//double c21= 0; // consumption matrix parameter
//double c22= 0.5; // consumption matrix parameter
double k_g= 1; // carrying capacity of prey: good year
double k_h= 1; // carrying capacity of prey: harsh year
...
int main (void)
{
	// Initialize parameters
		...
		double *p= d_vector(3);
		double *dfdt= d_vector(3);
	// double **log= d_matrix(T,4); //create log matrix
	// Creating log file
		FILE *out;
		out= fopen("fluctuating_onePrey.txt","w");
	// Initial Condition
		p[i_N1]= 0.01;
		p[i_N2]= 0.01;
		p[i_Pr1]= 1;
		state=1;
    fprintf(out, "time\tN1\tN2\tPr1\n");
    fprintf(out, "%lf\t%lf\t%lf\t%lf\n", t, p[i_N1], p[i_N2], p[i_Pr1]);
/*
...
*/
	...
	for (i=1;i<=T;i++){//create for loop
		...
		if(p[i_N1]<0||p[i_N2]<0||p[i_Pr1]<0) {
...
double dN1_dt(double time, double vr[])
{return b1*vr[i_N1]*vr[i_Pr1]*c-d1*vr[i_N1];}
double dN2_dt(double time, double vr[])
{return b2*vr[i_N2]*vr[i_Pr1]*c-d2*vr[i_N2];}
double dPr1_dt_g(double time, double vr[])
{return a1*vr[i_Pr1]*(1-vr[i_Pr1]/k_g)-vr[i_Pr1]*c*(vr[i_N2]+vr[i_N1]);}  // good year
double dPr1_dt_h(double time, double vr[])
{return a1*vr[i_Pr1]*(1-vr[i_Pr1]/k_h)-vr[i_Pr1]*c*(vr[i_N2]+vr[i_N1]);}  //harsh year
