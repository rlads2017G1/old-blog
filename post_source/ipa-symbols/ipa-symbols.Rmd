...
Solving the first task (IPA symbol) is easy, draing syntax trees is hard and I haven't have a solution yet[^tree].

## Typing IPA Symbols

There are two problems to be solved in order facilitate using IPA symbols in R Markdown:
...
To define a new font family for IPA symbols, set `header.tex` and include it in the yaml header of R Markdown document:
```tex
% Set font size
...
[^me]: I actually stared and gave up learning programming languages for three times (C++, C, and then Python) before I successfully learned R.